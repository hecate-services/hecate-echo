...
-module(hecate_echo_limiter_tests).
...
per_caller_allows_up_to_its_own_max() ->
    ...
    Results = [hecate_echo_limiter:allow(Caller) || _ <- lists:seq(1, 20)],
    ?assertEqual(lists:duplicate(20, allow), Results).

per_caller_denies_once_over_its_own_max() ->
    Caller = unique_caller(),
    [hecate_echo_limiter:allow(Caller) || _ <- lists:seq(1, 20)],
    ?assertEqual(deny, hecate_echo_limiter:allow(Caller)),
    ?assertEqual(deny, hecate_echo_limiter:allow(Caller)).

distinct_callers_have_independent_counters() ->
    CallerA = unique_caller(),
    CallerB = unique_caller(),
    [hecate_echo_limiter:allow(CallerA) || _ <- lists:seq(1, 20)],
    ?assertEqual(deny, hecate_echo_limiter:allow(CallerA)),
    %% CallerB's own budget is untouched by CallerA's burst.
    ?assertEqual(allow, hecate_echo_limiter:allow(CallerB)).
...
the_global_key_has_its_own_higher_max() ->
    ...
    ?assertEqual(lists:duplicate(21, allow),
                 [hecate_echo_limiter:allow(K) || K <- GlobalResults]).
...
exceeding_the_global_max_denies_further_unattributed_calls() ->
    ...
    %% a unit test to.
    Results = [hecate_echo_limiter:allow('$global') || _ <- lists:seq(1, 400)],
